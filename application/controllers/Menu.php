	...
	public function followers(){
		if(!$this->session->userdata('credentials')):
			...
			if($this->db->query("select poin from instagram where id='".$_session['user_id']."'")->row()->poin < 1):
				...
				$info1 = $this->curl->simple_get('https://instagram.com/api/v1/users/'.$_session['user_id'].'/info/');
				if(json_decode($info1)->user->is_private)
					...
					$this->load->model('instaloader');
					$info2 = $this->curl->simple_get('https://instagram.com/api/v1/users/'.$_session['user_id'].'/info/');
	...
	public function likes(){
		if(!$this->session->userdata('credentials')):
			...
			if($this->db->query("select poin from instagram where id='".$_session['user_id']."'")->row()->poin < 1):
				...
				$this->load->library('curl');
				$info = $this->curl->simple_get('https://instagram.com/api/v1/users/'.$_session['user_id'].'/info/');
